			...
			if(res.err == 0){
				try{
					...
					$.each(json.list_arr, function(i,item){
						...
						s += '<div class="media-body"><p class="media-heading clearfix"><small class="pull-right text-muted">'+item.rates+'<span class="break">|</span>'+item.date+'</small><span class="text-info">'+item.author+' '+item.user_level+'</span></p><p class="text-muted small">'+item.content+'</p>';
						if(item.content_zhui){
							s += '<p class="text-muted"><span class="label label-info-border normal">'+item.zhui_time+'追评</span> '+item.content_zhui+'</p>';
						}
...
var History = {
	...
	'List': function(){
		...
		if (this.Json) {
			...
			var jsonstr = $.cookie('view_history');
	...
	'Insert': function(title, price, d_price, limit, days){
		var jsondata = $.cookie('view_history');
		...
		$.cookie('view_history', jsonstr,{path:'/',expires:days});
	}
};
