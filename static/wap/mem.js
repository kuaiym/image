                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (window.ppData.err == 0) {
                                layer.msg('会员组升级成功，请稍后！', {icon: 1, time: 2000, shade: 0.6}, function () {
    ...
    'order': {
        'init': function () {
            ...
            ppAjax.submit("#change_form", function (data) {
                ppAjax.alert(data);
                if (window.ppData.err == 0) {
                    layer.msg(window.ppData.msg, {icon: 1, time: 2000, shade: 0.6}, function () {
                        window.location.reload();
                    });
                } else {
            ...
            ppAjax.submit("#tx_form", function (data) {
                ...
                if (window.ppData.err == 0) {
                    layer.msg(window.ppData.msg, {icon: 1, time: 2000, shade: 0.6}, function () {
                        window.location = 'index.php?u=record-index-type-3';
                    });
                } else {
                    layer.msg(window.ppData.msg, function () {
                    });
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(_qqlist, function (n, item) {
                        qqlist += '<p><a class="text-info" target="_blank" href="mqqwpa://im/chat?chat_type=wpa&uin=' + item.qq + '&version=1&src_type=web&web_src=' + weburl + '"><i class="icon-qq"></i> [客服] ' + item.name + '</a></p>';
                    });
            ...
            if (buyer_type == 1) {
                ...
                if (type == 2) {
                    ...
                    layer.confirm(notice, {icon: 3}, function (index) {
                        ppAjax.post("index.php?u=tuikuan-add-ajax-1", {
                            "order_id": order_id,
                            "tui_price": tui_price,
                            "content": content,
                            "type": type
    ...
    'pro': {
        'change_val': function (obj, id, cid, type) {
            ...
            input.blur(function () {
                ...
                if (newtxt != txt) {
                    obj.html(newtxt);
                    ppAjax.post("index.php?u=products-ajaxset-ajax-1", {
                        "id": id,
                        "type": type,
                        "cid": cid,
                        "txtvalue": newtxt
        ...
        'flag_submit': function (pid, cid, flags, days) {
            layer.confirm('确认购买此广告位？', {icon: 3, title: '确定？'}, function (index) {
                ppAjax.post("index.php?u=products-setflag-ajax-1", {
                    "id": pid,
                    "cid": cid,
                    "flags": flags,
                    "days": days
        ...
        'refresh': function (id, cid) {
            layer.confirm('分类页面置顶效果！<br/>扣除<b class="text-red">' + jf_refresh + jf_name + '</b>/次', {
                icon: 3,
                title: '确定？'
            }, function (index) {
...
(function ($) {
    ...
    var usr_showmenu = function () {
        $("#usr_showmenu").on('click', function (e) {
            layer.open({content: $("#usr_leftmenu").html(), type: 1});
            e.preventDefault();
        });
    };
