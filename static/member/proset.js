...
var kpUpload = {
	...
	boxAdd : function(file, big, thumb, aid) {
		...
		var d_li = $('<div class="col-sm-2 d_li" id="photo_'+aid+'" pid="'+aid+'"></div>').prepend(thumbnail);
	...
	fileboxAdd : function(file,aid,size,name){
		var inps = '<div class="col-xs-8"><input type="hidden" name="downs['+aid+'][aid]" value="'+aid+'"><input type="hidden" class="form-control" name="downs['+aid+'][down_url]" value="'+file+'" readonly="readonly"><input type="text" class="form-control" name="downs['+aid+'][title]" value="'+name+'"></div>';
			inps += '<div class="col-xs-3"><div class="input-group"><input name="downs['+aid+'][size_mb]" value="'+size+'" type="text" class="form-control" /><span class="input-group-addon">MB</span></div></div>'; 
			inps += '<div class="col-xs-1"><a href="javascript:;" class="btn btn-danger" aid="'+aid+'" onclick="file_del('+aid+');">删除</a></div>'; 
...
$(document).ready(function(){
	...
	var myos = appInfo();
	if(myos.msie && myos.version < 9){
		$("#browser").html('<strong style="color:red"><i class="icon-bullhorn text-muted"></i> 请使用火狐或chrome浏览器编辑！</strong>').show('fast');
	}else{
		$("#browser").hide();
	}
	checkimages_num();
	...
	ppLoadJs(UEDITOR_HOME_URL+"ueditor.config.js", UEDITOR_HOME_URL+"ueditor.all.min.js", UEDITOR_HOME_URL+"lang/zh-cn/zh-cn.js", UEDITOR_HOME_URL+"third-party/zeroclipboard/ZeroClipboard.min.js", function(){
		...
		ue = UE.getEditor("content", {
            ...
            'bold', 'italic', 'underline', 'removeformat', 'autotypeset', 'forecolor', 'fontsize', '|',
